...
namespace HookTest
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //}
            //fs2 = new FileStream(savePath + full, FileMode.OpenOrCreate);
            string path =@"D:\LH\U3D\0A完成的项目源码\201906\SFS-YTYHX\Assets\StreamingAssets\video\白腔.mp4";
        ...
        public static void SaveFile(DataPack dp)
        {
            string savePath = @"F:\新建文件夹 (2)\";
            ...
            Console.WriteLine(s+"-"+full+"-"+start+"-"+end+"-"+len);
            if (start==0)
            {
                fs2 = new FileStream(savePath + full, FileMode.OpenOrCreate);
            }
            fs2.Write(msg,0,msg.Length);

            if (end==len)
            {
                fs2.Dispose();
                fs2.Close();
                Console.WriteLine("接受结束");
            }
